                ...
                def actor_loss(params):
                    ...
                    if cfg.reverse_kl:
                        ...
                        if cfg.faithful_same_point:
                            ...
                            # sample_and_log_prob call this branch used to make, and
                            # is bit-identical to it (scripts/msweep_audit/bitcheck.py).
                            # seed=akey, not seed=key: akey IS key unless
                            # fresh_minibatch_key is set, so defaults are unchanged.
                            # NOTE: y_estep_i stays None on the faithful_same_point
                            # path above, so combining faithful_same_point with
                            # sqrt_rho != 1.0 is unsupported and fails loudly at
                            # trace time rather than silently mis-fitting.
                            ...
                            if cfg.actor_update_mode == "weighted_mle":
                                ...
                                y_estep_i, _lp_base_i = (
                                    _old_pi.distribution.sample_and_log_prob(
                                        seed=akey, sample_shape=(n_estep,)
                                    )
                                )
